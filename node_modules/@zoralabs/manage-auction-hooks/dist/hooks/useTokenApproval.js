...
function useTokenApproval(contractAddress, tokenId, spender) {
    const { account, library } = core_1.useWeb3React();
    const contract = react_1.useMemo(() => {
    ...
    const [approved, setIsApproved] = react_1.useState(undefined);
    const [owned, setIsOwned] = react_1.useState(undefined);
    const loadApproval = react_1.useCallback(async () => {
        setIsApproved(undefined);
        // TODO(iain): add error reporting
        if (account && spender && contract && tokenId !== undefined) {
            setIsOwned(addresses_1.addressesMatch(await contract.ownerOf(tokenId), account));
