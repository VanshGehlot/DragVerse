...
function useEagerConnect() {
    ...
    const { connectors } = react_1.useContext(config_1.Web3ConfigurationContext);
    const [tried, setTried] = react_1.useState(false);
    const attemptEagerInjected = react_1.useCallback(async () => {
        if (!connectors) {
            return;
        }
    ...
    const attemptEagerDefault = react_1.useCallback((connector) => {
        return activate(connector, undefined, true).catch(() => {
            setTried(true);
        });
    }, [activate]);
    const attemptEager = react_1.useCallback((connector) => {
    ...
    react_1.useEffect(() => {
        ...
        delay(500).then(() => {
            const connector = connectors_1.fetchLastConnectorType(connectors);
            if (connector) {
